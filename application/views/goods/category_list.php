                ...
                <div class="page-title-box page-title-box--location">
                    <div class="page-title">
                        <ul class="page-title-list">
                            ...
                            <?if($cate_gb=='L'){?>
                                <li class="page-title-item active"><?=$category['CATE_NAME3']?> <em class="bold_yel">(<?=number_format($totalCnt)?>)</em></li> <!-- 퐐성화시 클래스 active추가 -->
                            <?}else if($cate_gb=='M'){?>
                                <li class="page-title-item"><a href="/category/category_list?cate_cd=<?=$category['CATE_CODE2']?>&cate_gb=L"><?=$category['CATE_NAME2']?></a></li>
                                <li class="page-title-item active"><?=$category['CATE_NAME3']?> <em class="bold_yel">(<?=number_format($totalCnt)?>)</em></li> <!-- 퐐성화시 클래스 active추가 -->
                            <?}else if($cate_gb=='S'){?>
                                <li class="page-title-item"><a href="/category/category_list?cate_cd=<?=$category['CATE_CODE1']?>&cate_gb=L"><?=$category['CATE_NAME1']?></a></li>
                                <li class="page-title-item"><a href="/category/category_list?cate_cd=<?=$category['CATE_CODE2']?>&cate_gb=M"><?=$category['CATE_NAME2']?></a></li>
                                <li class="page-title-item active"><?=$category['CATE_NAME3']?> <em class="bold_yel">(<?=number_format($totalCnt)?>)</em></li> <!-- 퐐성화시 클래스 active추가 -->
                            <?}?>
                        </ul>
                    </div>
                                            ...
                                            <ul id="srp-cate" class="srp-cate2">
                                                ...
                                                if( $cate_gb=='S' ){
                                                    foreach($arr_cate3 as $c3){?>
                                                        <li><a href="#none" onclick="search_goods('C','<?=$c3['CD']?>');" <?=($category['CATE_CODE3']==$c3['CD'])?'class="on"':''?>><?=$c3['NM']?></a></li>
                                                    <?}
                                                //대카테고리
                                                } else if( $cate_gb=='L' || $cate_gb=='M' ){
                                                    foreach($arr_cate2 as $c2){?>
                                                        <li><a href="#none" <?=(($cate_gb=='M')&&($category['CATE_CODE3']==$c2['CD']))?'class="on"':''?>><?=$c2['NM']?></a>
                                                            <ul>
                                                                <?foreach($arr_cate3 as $c3){
                                                                    if($c3['P_CD']==$c2['CD']){?>
...
<script>
    $(document).ready(function(){
         // 스크롤 시 상품상세 탭 네비 동작
        var tabsOffset = $('.filter-list').offset().top + 0 // - Scroll Up
        var tabsOffset_up = $('.filter-list').offset().top + 55; // - Scroll Down
