...
<div class="content">
    ...
    <div class="join-content-wrap">
        ...
        <form method="post" id="memForm" name="memForm" action="">
            <input type="hidden" id="chk_kind" name="sns_kind" value="<?=$this->session->userdata('sns_kind')?>" />
            <input type="hidden" id="chkId" name="sns_id" value="<?=$sns_id?>" />
            <input type="hidden" id="chk_email" name="chk_email" value="<?=$this->session->userdata('data_email')?>" />
            <input type="hidden" id="chk_auth" name="sns_nm" value="<?=$this->session->userdata('data_nickname')?>" />
            <input type="hidden" id="chk_rcmdId" name="chk_rcmdId" value="" />

            <div class="join-form">
                <?if(empty($this->session->userdata('data_email'))){?>
                    <div class="form-line form-line--wide form-line--cols">
                        <div class="form-line-info form-line-info--email">
                            <div class="form-line--cols-item">
                                <label><input type="text" name="mem_email1" class="input-text" placeholder="이메일" onkeyup="this.value=this.value.replace(/\s/g,'')"></label>
                            </div>
                            <span class="dash">@</span>
                            <input type="hidden" id="formJoin05_2" name="mem_email2"  onkeyup="this.value=this.value.replace(/\s/g,'')" value=""/>
                        ...
                        <!--<ul class="text-list">
                            <li class="text-item">인증메일을 수신하지 못하셨다면 스팸메일함을 확인해주시거나 <a href="mailto:info@etah.co.kr" class="bold">info@etah.co.kr</a>로 문의해주시면 고맙겠습니다.</li>
                        </ul>-->
                    </div>
                <?}?>
                ...
                <div class="form-line form-line--modify">
                    <div class="form-line-info">
                        ...
                        <input type="text" class="input-text" id="joinNameForm" name="mem_name" maxlength="30" placeholder="한글 15자, 영문 30자까지 입력 가능" onkeyup="this.value=this.value.replace(/\s/g,'')">
                    </div>
                </div>
            ...
            <div class="agree-info-area">
                <ul class="text-list">
                    <li class="text-item">주문 및 배송관련 정보는 수신동의와 상관없이 자동 발송되며, <br>
                        수신동의를 하게되시면 에타홈에서 제공하는 유용한 쇼핑정보(이벤트/할인쿠폰/기획전)소식을 받으실 수 있습니다.</li>
                </ul>
...
//        if(memForm.chk_email.value == ""){
//            alert("이메일 중복확인을 하셔야 합니다.");
//            memForm.chk_email.focus();
//            return false;
//        }
        ...
        if(memForm.sns_id.value == ""|| memForm.sns_id.value == null){
            alert("계정 연동오류. SNS 계정을 재로그인해 시도해주십시오");
            return false;
        }

        <?if(empty($this->session->userdata('data_email'))){?>
        var mem_email1 = $("input[name=mem_email1]").val();
        var mem_email2 = $("input[name=mem_email2]").val();

        if (mem_email1 == "") {
            alert("회원 이메일을 입력하지 않았습니다.");
            $("input[name=mem_email1]").focus();
            return false;
        }
        ...
        var mem_email = mem_email1+"@"+mem_email2;
        $('#chk_email').val(mem_email);
    ...
    {
        var SSL_val = "<?=$_SERVER['HTTP_HOST']?>";
        var frm = document.getElementById('memForm');
        if(!jsCheck()){
            return false;
        }
        ...
        $.ajax({
            type: 'POST',
            url: 'https://'+SSL_val+'/member/sns_join_finsih',
            dataType: 'json',
            data: param,
            error : function(res) {
                alert('Database Error');
            ...
            success: function(res) {
                if(res.status == 'ok'){
                    frm.action = "https://"+SSL_val+"/member/join_finish";
                    frm.submit();
                }
                else {
                    alert(res.message);
//                    location.href = "https://"+SSL_val+"/member/login";
                }
            }
        });
    }
